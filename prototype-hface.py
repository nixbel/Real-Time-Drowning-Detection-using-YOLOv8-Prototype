...
@st.cache_resource
def load_model():
    # Automatically download model from Hugging Face
    model_path = hf_hub_download(
        repo_id="tonett/drowningv1",   # Your Hugging Face repo
        filename="best.pt",            # Model filename
        repo_type="model"              # Specify this to access models
    )
...
# ==============================
class_colors = {
    "drowning": (0, 165, 255),  # Orange (BGR)
    "swimming": (255, 0, 0),    # Blue (BGR)
}
...
if mode == "Upload Video":
    ...
    if uploaded_file is not None:
        ...
        while cap.isOpened():
            ...
            # Run YOLO detection
            results = model(frame, device="cuda" if torch.cuda.is_available() else "cpu", stream=True)
...
# ==============================
# Mode 2: Live RTSP Feed with YOLO + Recording + Alarm
# ==============================
elif mode == "Live RTSP Feed":
    st.title("📹 Real-time YOLO Detection from Tapo RTSP (GPU Enabled)")
    stframe = st.empty()
    ...
    if not cap.isOpened():
        ...
        while cap.isOpened():
            ...
            if drowning_detected:
                alert_placeholder.error("🚨 Someone is DROWNING! 🚨")
                st.markdown(
                    """
                    <audio autoplay>
